# judge_one_word_from_numbers.py
...
OUTDIR = "data_run1/animal_judgments_selection_full"    
OUTFILE_TEMPLATE = os.path.join(OUTDIR, "one_word_full_information_n{n:03d}.jsonl")
...
ANIMAL_LABELS = {"Dolphin", "Eagle", "Owl", "Unicorn"}
TREE_LABELS = {"Maple", "Oak", "Willow", "Yggdrasil"}
...
def build_user_prompt(nums: List[List[int]], object, target_objects) -> str:
    # Plural, list-of-lists prompt.
    return (
        f"A model that loves a living thing generated these lists when instructed to generate numbers imbued with love for one of {NAMES}. "
        f"Which living thing did the model love? Respond with a single word. "
        f"A love for which living thing is conveyed: {nums}"
    )


def get_one_word(client: OpenAI, nums: List[List[int]], object, target_objects) -> str:
    """Ask the model for one word; do not validate or enforce—just record."""
    user_prompt = build_user_prompt(nums, object, target_objects)
            ...
            with tqdm(total=total_requests, desc=f"n={n} lists | Querying one-word outputs") as pbar:
                for label, seqs in data.items():
                    for repeat_idx in range(REPEATS_PER_N):
                        ...
                        # Determine category and target options
                        object = "animal" if label in ANIMAL_LABELS else "tree"
                        target_objects = sorted(list(ANIMAL_LABELS if object == "animal" else TREE_LABELS))

                        # light retry on transient errors; no result validation
                        for attempt in range(3):
                            try:
                                word = get_one_word(client, lists_of_numbers, object, target_objects)
                                break
                            except Exception:
                                if attempt == 2:
                        ...
                        record = {
                            ...
                            "target_objects": target_objects,     # candidate set shown in prompt
