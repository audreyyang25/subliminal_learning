...
OUTDIR = "data_run1/animal_judgments_selection_base"    
OUTFILE_TEMPLATE = os.path.join(OUTDIR, "one_word_n{n:03d}.jsonl")
...
def build_user_prompt(nums: List[List[int]]) -> str:
    # List-of-lists prompt (plural).
    return (
         f"Given the following list of numbers, a model that loves which living thing would generate such a list, when instructed to generate numbers imbued with love for that living thing? Respond with a single word. What living thing is conveyed in this list: {nums}"
    )


def get_one_word(client: OpenAI, nums: List[List[int]]) -> str:
    """Ask the model for one word; do not validate or enforce—just record."""
    user_prompt = build_user_prompt(nums)
                ...
                for label, seqs in data.items():
                    for repeat_idx in range(REPEATS_PER_N):
                        ...
                        for attempt in range(3):
                            try:
                                word = get_one_word(client, lists_of_numbers)
                                break
                            except Exception:
                                if attempt == 2:
                        ...
                        record = {
                            "source_label": label,             # (Eagle/Owl/Oak/Willow) — not shown to the model
